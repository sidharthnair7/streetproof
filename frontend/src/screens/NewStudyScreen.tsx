  ...
  return (
    <div className="max-w-[1000px] mx-auto space-y-6">
      <div>
        ...
        <p className="mt-3 max-w-[600px] text-[13px] leading-6 text-[#536363]">
      ...
      <div className="flex max-w-[640px] items-center pt-2">
        {steps.map((s, i) => (
          <div key={s} className="flex flex-1 items-center">
            <div className={`flex items-center gap-2 text-[13px] font-semibold ${step >= i ? 'text-[#126b6a]' : 'text-[#576361]'}`}>
              <span className={`grid h-8 w-8 place-items-center rounded-full border text-[12.5px] font-bold ${step > i ? 'border-[#3bb9a3] bg-[#d9f3e9] text-[#237568]' : step === i ? 'border-[#1b7a74] bg-[#1b7a74] text-white shadow-md' : 'border-[#d3dedd] bg-white text-[#576361]'}`}>
      ...
      {error && (
        <div className="flex items-start gap-2 rounded-xl border border-[#f1c8bd] bg-[#fdf1ee] px-4 py-3 text-[13px] text-[#9b3d2a]">
          ...
          <div className="panel rounded-2xl p-6 space-y-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-[12.5px] text-[#556463]">VS13 sample clips: each car held a known speed on cruise control. One click runs it with the calibration stored on the DKG.</p>
              </div>
              <FileVideo size={22} className="shrink-0 text-[#457770]" />
            </div>
            <div className="max-h-[380px] space-y-2 overflow-y-auto pr-1">
              {samples.map((clip) => (
                ...
                >
                  <div>
                    <div className="text-[13px] font-semibold text-[#18232a]">{clip.title}</div>
                    <div className="mono text-[12px] text-[#576363]">{clip.name}</div>
                  </div>
                  {busy === clip.name ? <Loader2 size={15} className="animate-spin text-[#1b7a74]" /> : <Play size={14} className="text-[#1b7a74]" />}
                </button>
              ))}
              {samples.length === 0 && <div className="text-[13px] text-[#556463]">No sample clips found in the server's samples folder.</div>}
            </div>
            <div className="text-[12px] leading-relaxed text-[#586362]">{samples[0]?.credit}</div>
          </div>
            ...
            <div className="space-y-2.5 text-[12.5px] text-[#526463]">
              {[
                ...
                <div key={t} className="flex gap-2.5">
                  <Check size={14} className="mt-0.5 shrink-0 text-[#297b6d]" />
          ...
          <div className="panel rounded-2xl p-6 space-y-4">
            ...
            <div className="grid gap-2 sm:grid-cols-2">
              {([
                ...
                <button key={id} onClick={() => setMode(id)} className={`flex items-start gap-3 rounded-xl border p-3 text-left transition-colors ${mode === id ? 'border-[#1b7a74] bg-[#effaf5]' : 'border-[#dce5e3] bg-white hover:bg-[#f6faf8]'}`}>
                  ...
                  <span>
                    <span className="block text-[13px] font-semibold text-[#18232a]">{title}</span>
                    <span className="block text-[12.5px] text-[#556463]">{sub}</span>
                  </span>
                </button>
              ))}
            </div>
            ...
            {mode === 'dkg' && (
              <div className="space-y-3">
                {calibrations.length === 0 ? (
                  <div className="text-[13px] text-[#9b3d2a]">No calibration is published yet.</div>
                ) : (
                  <>
                    <select value={calibrationRef} onChange={(e) => setCalibrationRef(e.target.value)} className="w-full rounded-lg border border-[#dce5e3] bg-white px-3 py-2 text-[13px]">
                      {calibrations.map((c) => (
                        <option key={c.id} value={c.ual ?? ''}>{c.cameraLabel} · focal {c.focalPx} px · {c.passes?.length ?? 1} pass{(c.passes?.length ?? 1) === 1 ? '' : 'es'}</option>
                      ))}
                    </select>
                    <div className="rounded-xl bg-[#fff6ec] px-4 py-2.5 text-[12.5px] text-[#8a5a2b]">
                    ...
                    {chosen && (
                      <div className="rounded-xl bg-[#f6faf8] px-4 py-3 text-[12.5px] text-[#516462]">
                        <div className="mono break-all text-[12px] text-[#257b6d]">{chosen.ual}</div>
                    ...
                    <label className="flex items-center justify-between gap-3 text-[13px] text-[#40585a]">
            ...
            {mode === 'curb' && (
              <div className="space-y-3">
                {study.links.firstFrame ? (
                  <div className="relative overflow-hidden rounded-xl border border-[#dce5e3]">
                    ...
                    {marks.map((m, i) => (
                      <span
                        key={i}
                        className="pointer-events-none absolute grid h-6 w-6 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full border-2 border-white bg-[#1b7a74] text-[12px] font-bold text-white shadow"
                  ...
                  <div className="text-[13px] text-[#556463]">The first frame is not available for this video.</div>
                )}
                <label className="flex items-center justify-between gap-3 text-[13px] text-[#40585a]">
                ...
                <div className="text-[12.5px] text-[#556463]">{marks.length < 2 ? `Click point ${marks.length + 1} on the near edge of the road.` : 'Two marks set. Click again to start over.'}</div>
              </div>
            )}
            ...
            {mode === 'none' && (
              <div className="rounded-xl bg-[#fff6ec] px-4 py-3 text-[13px] text-[#8a5a2b]">Every vehicle will be refused with NO_CALIBRATION. That is the point: without a known distance there is no honest speed.</div>
            )}
          </div>

          <div className="panel rounded-2xl p-6 space-y-4">
            <h3 className="text-[15px] font-semibold text-[#18232a]">About this street</h3>
            <label className="block text-[13px] text-[#40585a]">
              Street name
            ...
            <label className="flex items-start gap-3 rounded-xl border border-[#dce5e3] bg-white px-3 py-2.5 text-[13px] text-[#40585a]">
            ...
            <div className="mono break-all rounded-lg bg-[#f6faf8] px-3 py-2 text-[12px] text-[#516462]">video sha256 {short(study.videoSha256, 16, 10)}</div>
          </div>
        </div>
      )}
        ...
        <div className="panel max-w-[780px] rounded-2xl p-6 md:p-8 space-y-5">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="mt-1 text-[13px] text-[#536463]">Every vehicle must pass all of these, or it is refused with the reason. Refused vehicles never enter the 85th percentile.</p>
            </div>
              ...
              <div key={g.code} className="flex items-center justify-between gap-4 py-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-[12.5px] text-[#556464]">{g.rule}</div>
                  </div>
                </div>
                <span className="mono text-[12px] text-[#586362]">{g.code}</span>
              </div>
            ))}
          </div>
        </div>
