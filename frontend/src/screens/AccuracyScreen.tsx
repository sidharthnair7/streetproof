  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-3 max-w-[680px] text-[13px] leading-6 text-[#536463]">
      ...
      {report.clips === 0 ? (
        <div className="panel rounded-2xl p-6 text-[13px] text-[#536363]">
        ...
        <>
          <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            <BentoCard glowColor="rgba(59, 185, 163, 0.3)">
              ...
              <div className="mt-1 text-[12.5px] text-[#556463]">{report.refused} refused</div>
            </BentoCard>
            <BentoCard glowColor="rgba(236, 138, 69, 0.3)">
              <div className="eyebrow mb-1 text-[#a45c26]">Mean error</div>
              <div className="stat-number text-3xl font-medium text-[#a45c26]">{report.meanAbsErrorPercent != null ? <AnimatedCounter value={report.meanAbsErrorPercent} decimals={1} suffix="%" /> : '–'}</div>
              <div className="mt-1 text-[12.5px] text-[#556463]">Absolute, across the unseen clips</div>
            </BentoCard>
            <BentoCard glowColor="rgba(236, 80, 80, 0.25)">
              <div className="eyebrow mb-1 text-[#b84e31]">Worst clip</div>
              <div className="stat-number text-3xl font-medium text-[#b84e31]">{report.maxAbsErrorPercent != null ? `${report.maxAbsErrorPercent}%` : '–'}</div>
              <div className="mt-1 text-[12.5px] text-[#556463]">Telraam states about 10% for its sensor</div>
            </BentoCard>
            <BentoCard glowColor="rgba(53, 102, 174, 0.3)">
              ...
              <div className="mt-1 text-[12.5px] text-[#556463]">{current?.spreadPercent != null ? `Agreed within ${current.spreadPercent}%` : 'Loaded from the DKG'}</div>
            </BentoCard>
          </section>
          ...
          <section className="panel overflow-hidden rounded-2xl">
                  ...
                  {[...tests, ...passes].map((r) => (
                    <tr key={r.studyId} onClick={() => openStudy(r.studyId)} className="cursor-pointer hover:bg-[#f6faf8]">
                      ...
                      <td className="px-4 py-3">
                        {r.role === 'test' ? (
                          <span className="rounded-full bg-[#e7eef9] px-2 py-0.5 text-[12px] font-semibold text-[#3e67a6]">unseen</span>
                        ) : (
                          <span className="rounded-full bg-[#edf0ef] px-2 py-0.5 text-[12px] font-semibold text-[#596262]">calibration pass</span>
                        )}
                      </td>
                      <td className="px-4 py-3 text-[#546462]">{r.knownKmh} km/h</td>
                      ...
                      <td className="px-4 py-3">
                        {r.errorPercent != null && (
                          <div className="flex items-center gap-3">
                            ...
                            <span className="mono w-14 text-right text-[12.5px] text-[#40585a]">{r.errorPercent > 0 ? '+' : ''}{r.errorPercent}%</span>
                          </div>
                        )}
                      </td>
                    </tr>
            ...
            <div className="border-t border-[#e5ebea] px-6 py-3 text-[12.5px] text-[#556463]">
          ...
          {cross && (
            <section className="grid gap-6 xl:grid-cols-[.9fr_1.1fr]">
              <div className="panel rounded-2xl p-6">
                  ...
                  {cross.levels.map((l) => (
                    <div key={l.passes}>
                      <div className="mb-1 flex items-baseline justify-between text-[13px]">
                        <span className="font-semibold text-[#18232a]">{l.passes} pass{l.passes === 1 ? '' : 'es'} <span className="font-normal text-[#576363]">({l.combinations} choices)</span></span>
                ...
                <p className="mt-5 text-[12.5px] leading-relaxed text-[#556463]">{cross.method}</p>
              </div>
              <div className="panel rounded-2xl p-6">
                ...
                <div className="mt-4 space-y-1.5">
                  {cross.singleClipChoices.map((c) => (
                    <div key={c.clip} className="flex items-center gap-3 text-[13px]">
                ...
                <p className="mt-4 text-[12.5px] leading-relaxed text-[#556463]">
            ...
            <div key={c.id} className="rounded-xl border border-[#e3eae8] bg-white p-4">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  ...
                  <div className="mono mt-0.5 break-all text-[12px] text-[#257b6d]">{c.ual}</div>
                </div>
                <div className="text-right text-[12.5px] text-[#516462]">
              ...
              {c.passes && c.passes.length > 0 && (
                <div className="mt-3 grid gap-2 sm:grid-cols-3">
                  {c.passes.map((p) => (
                    <div key={p.videoSha256} className="rounded-lg bg-[#f6faf8] px-3 py-2 text-[12.5px] text-[#516462]">
                      ...
                      <div className="mono mt-1 truncate text-[12px]" title={p.studyUal ?? ''}>{p.studyUal ? short(p.studyUal, 16, 10) : `sha ${short(p.videoSha256, 10, 6)}`}</div>
                    </div>
                  ))}
                </div>
              )}
              {c.ual === report.calibrationUal && (
                <div className="mt-3 inline-flex items-center gap-1.5 rounded-full bg-[#d9f3e9] px-2.5 py-1 text-[12px] font-semibold text-[#287463]"><Check size={11} /> Used by the held-out test above</div>
              )}
            </div>
          ))}
          {calibrations.length === 0 && <div className="text-[13px] text-[#556463]"><Target size={13} className="mr-1 inline" /> No calibration published yet.</div>}
        </div>
      </section>
    </div>
  )
